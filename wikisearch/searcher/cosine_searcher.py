...
class CosineSearch(SearchEngineBase):
    def __init__(self, inverted_index_file, docvec_index_file, meta_file):
        start = time.time()
        self.invertedIndex = LoadInvertedIndex(inverted_index_file)
        self.DocVecIndex = LoadDocVecIndex(docvec_index_file)
        self.article_mata = load_meta(meta_file)
        elapsed = time.time() - start
        logging.info("Build CosineSearch Engine in %f secs" % elapsed)
        return
    ...
    def search(self, query, dump):
        ...
        for doc_id in result:
            score[doc_id] = cosine_similarity(query, self.DocVecIndex[doc_id],
                                              self.invertedIndex, N)
