...
def tf_idf_score(query_terms, doc_id, inverted_index, N):
    re = 0.0
    epsilon = 1e-8
    for term in query_terms:
        if not term in inverted_index:
            continue
        re += (1 + math.log(epsilon + inverted_index[term][doc_id]) /
               math.log(10)) * math.log(N * 1.0 / len(inverted_index[term]))
    return re
